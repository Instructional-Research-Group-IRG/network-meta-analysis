...
NNMA_MVmodel_main <- rma.mv(yi = effect_size, 
                            V = V_list, 
                            random = ~1 | record_id/es_id,
                            mods = ~ NL_TX + TES_TX + VF_TX + RS_TX + FF_TX + dosage_overall_hours_avg + group_size_category + interventionist_numeric + ongoing_training + research_lab - 1, 
                            test = "t",
                            data = NNMA_Data_Subset_grpID,
                            method = "REML")
...
## Analysis 2: All Instructional Components plus control covariates 
NNMA_MVmodel_ALLICs <- rma.mv(yi = effect_size, 
                            V = V_list, 
                            random = ~1 | record_id/es_id,
                            mods = ~ NL_TX + TES_TX + VF_TX + RS_TX + FF_TX + N_TX + SEO_TX + TV_TX + RV_TX + FWOF_TX + GS_TX + MS_TX + PRM_TX + PRB_TX + control_nature_numeric + dosage_overall_hours_avg + group_size_category + measure_developer_numeric + interventionist_numeric + ongoing_training + research_lab - 1, 
                            test = "t",
                            data = NNMA_Data_Subset_grpID,
                            method = "REML")
summary(NNMA_MVmodel_ALLICs)
