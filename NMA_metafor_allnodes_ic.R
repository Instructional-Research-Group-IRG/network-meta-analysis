...
# Execute network meta-analysis using a contrast-based random-effects model using BAU as the reference condition: intervention_content == "Whole Numbers (W)"
  ...
  num_students_icW <- NMA_data_analysis_subset_grpID_icW %>% dplyr::select(record_id, contrast_id, intervention_content, measure_name, intervention_prelim, intervention_n, comparison_prelim, comparison_n, full_sample_size)
  print(num_students_icW, n=Inf)
  
  num_students_icW <- num_students_icW %>% group_by(record_id, contrast_id) %>% arrange(desc(intervention_n), .by_group = TRUE) %>% ungroup() # Arrange by descending intervention_n within each study-contrast so that the largest sample size within the contrast is selected below.
  print(num_students_icW, n=Inf)
  ...
  print(check_icW)
    ...
    res_mod_icW <- rma.mv(effect_size, V_list, 
                            mods = ~ 0 + FF + FF.RS + NL.FF.RS + NL.RS + RS + VF.FF.RS + VF.RS, # BAU is excluded to serve as the reference level for the comparisons.
    ...
    NMA_data_analysis_subset_grpID_icW_BAU2 <- NMA_data_analysis_subset_grpID_icW %>% filter(intervention_prelim!="FF")
    ...
    res_mod_icW_BAU2 <- rma.mv(effect_size, V_list_BAU2, 
                              mods = ~ FF.RS + NL.FF.RS + NL.RS + RS + VF.FF.RS + VF.RS - 1, # BAU is excluded to serve as the reference level for the comparisons.
                              random = ~ 1 | record_id/es_id, 
                              rho=0.60, 
    ...
    #                            mods = ~ factor(intervention_prelim) - factor(comparison_prelim), # BAU is excluded to serve as the reference level for the comparisons.
    #                            random = ~ 1 | record_id/es_id,
    #                            rho=0.60,
    #                            data=NMA_data_analysis_subset_grpID_icW)
    ...
    ### Test for inclusion of indirect evidence: FF VS BAU pairwise comparison
      ...
      NMA_data_analysis_subset_grpID_icW_FF <- NMA_data_analysis_subset_grpID_icW_BAU %>% filter(intervention_prelim=="FF" & comparison_prelim=="BAU")
    ...
    ### Test for inclusion of indirect evidence: FF+RS VS BAU pairwise comparison
      ...
      #### Fit NMA model assuming consistency (tau^2_omega=0) using variance instead of V_list
      NMA_data_analysis_subset_grpID_icW_FFRS <- NMA_data_analysis_subset_grpID_icW_BAU %>% filter(intervention_prelim=="FF+RS" & comparison_prelim=="BAU")
  ...
  # Execute network meta-analysis using a contrast-based random-effects model using BAU as the reference condition: intervention_content == "Rational Numbers (R)"
    ...
    print(num_students_icR_long2_1, n=Inf)
      ...
      #Note: Change intervention bundle back from "KEEP" to original value for groups of students that the team has determiend to not be the same group of students assigned to the same intervention bundle across multiple contrasts of the same study and thus not dropped just above.
    num_students_icR_long2_1 <- num_students_icR_long2_1 %>% mutate(intervention_comparison = if_else(contrast_id=="87223" & group_IC=="intervention_prelim", "NL+SE+FF+RS", intervention_comparison)) 
    num_students_icR_long2_1 <- num_students_icR_long2_1 %>% mutate(intervention_comparison = if_else(contrast_id=="89813" & group_IC=="intervention_prelim", "NL+RS", intervention_comparison)) 
    num_students_icR_long2_1 <- num_students_icR_long2_1 %>% mutate(intervention_comparison = if_else(contrast_id=="89198_disagg" & group_IC=="intervention_prelim", "NL+FF+RS", intervention_comparison)) 
    num_students_icR_long2_1 <- num_students_icR_long2_1 %>% mutate(intervention_comparison = if_else(contrast_id=="89179" & group_IC=="intervention_prelim", "NL+SE+FF+RS", intervention_comparison)) 
    num_students_icR_long2_1 <- num_students_icR_long2_1 %>% mutate(intervention_comparison = if_else(contrast_id=="90582_disagg_b" & group_IC=="intervention_prelim", "NL+FF+RS", intervention_comparison)) 
    print(num_students_icR_long2_1, n=Inf)
