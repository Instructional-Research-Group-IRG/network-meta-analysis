...
# Execute network meta-analysis using a contrast-based random-effects model using BAU as the reference condition
  ...
  # tabyl(NMA_data_analysis_subset_grpID_all$intervention_content...33)
  # NMA_data_analysis_subset_grpID_all <- NMA_data_analysis_subset_grpID_all %>% distinct(contrast_id, .keep_all = TRUE)
  # NMA_data_analysis_subset_grpID_all %>% count()
  ...
  num_contrasts_all <- NMA_data_analysis_subset_grpID_all %>% dplyr::select(record_id, contrast_id, intervention_prelim, comparison_prelim)
  print(num_contrasts_all)
  num_contrasts_all_long <- num_contrasts_all %>% pivot_longer(c(intervention_prelim, comparison_prelim ), names_to= "group_all", values_to="group_intervention")
  print(num_contrasts_all_long)
  num_contrasts_all_long2 <- num_contrasts_all_long %>% distinct(record_id, contrast_id, group_intervention, .keep_all = TRUE)
  print(num_contrasts_all_long2)
  tabyl(num_contrasts_all_long2$contrast_id) #Should be n=2 for each contrast if reshape and distinct steps done correctly: 1 intervention & 1 comparison per unique contrast. 
  num_contrasts_all_long3 <- tabyl(num_contrasts_all_long2$group_intervention)
  num_contrasts_all_long3 <- num_contrasts_all_long3 %>% dplyr::select(intervention= 'num_contrasts_all_long2$group_intervention', num_contrasts= 'n')
  str(num_contrasts_all_long3)
  num_contrasts_all_long3$intervention <- as.character(num_contrasts_all_long3$intervention)
  ...
  num_students_all2 <- num_students_all %>% distinct(record_id, contrast_id, .keep_all = TRUE) #Keep only unique entries of each unique study-contrast so that each group of students is not summed more than once (because of multiple measures within some contrasts).
  print(num_students_all2) 
  num_students_all_long <- num_students_all2 %>% pivot_longer(c(intervention_prelim, comparison_prelim ), names_to= "group_all", values_to="intervention_comparison") #Put intervention and comparison groups in same row for summing students receiving the same intervention bundle regardless of whether it was received in the intervention or comparison group assignment.
  print(num_students_all_long, n=20)
  num_students_all_long2 <- num_students_all_long %>% mutate(num_students_bundle= ifelse(group_all == "intervention_prelim", intervention_n, comparison_n)) # Put number of students by assignment group in same column condition on the intervention/comparison group assignment.
  print(num_students_all_long2)
  num_students_all_long3 <- num_students_all_long2 %>% group_by(intervention_comparison) %>% summarize(sum_num_students_bundle= sum(num_students_bundle)) # Sum students by intervention bundle.
  str(num_students_all_long3)
  print(num_students_all_long3)
  target_all <- c("BAU","FF","FF+RS","NL+FF+RS","NL+RS","NL+SE+FF+RS","NL+SE+RS","NL+SE+VF+RS","RS","SE+RS","VF+FF+RS","VF+RS")
  num_students_all_long3 <- num_students_all_long3[match(target_all, num_students_all_long3$intervention_comparison),]
  num_students_all_long3$intervention_comparison <- as.character(num_students_all_long3$intervention_comparison)
  str(num_students_all_long3)  
  print(num_students_all_long3)    
        
  ## Add contrast matrix to dataset
  NMA_data_analysis_subset_grpID_all <- contrmat(NMA_data_analysis_subset_grpID_all, grp1="intervention_prelim", grp2="comparison_prelim")
  str(NMA_data_analysis_subset_grpID_all)
  
  ## Calculate the variance-covariance matrix for multi-treatment studies
  V_list <- vcalc(variance, cluster= record_id, obs= measure_name, type= domain, rho=c(0.6, 0.6), grp1=group1_id, grp2=group2_id, w1=intervention_n, w2=comparison_n, data=NMA_data_analysis_subset_grpID_all)
  V_list    
  V_list_all <- data.frame(V_list)
  write_csv(V_list_all, 'V_list_all.csv')
        
  ##Run standard NMA with the unique interventions bundles as moderators  
  tabyl(NMA_data_analysis_subset_grpID_all$intervention_prelim)
  tabyl(NMA_data_analysis_subset_grpID_all$comparison_prelim)
  check_all <- NMA_data_analysis_subset_grpID_all %>% dplyr::select(record_id, contrast_id, intervention_prelim, comparison_prelim)
  print(check_all)

    ### Fit model assuming consistency (tau^2_omega=0)
    res_mod_all <- rma.mv(effect_size, V_list, 
                            mods = ~ FF + FF.RS + NL.FF.RS + NL.RS + NL.SE.FF.RS + NL.SE.RS + NL.SE.VF.RS + RS + SE.RS + VF.FF.RS + VF.RS - 1, # The "treatment" left out (BAU) becomes the reference level for the comparisons
                            random = ~ 1 | record_id/es_id, 
                            rho=0.60, 
                            data=NMA_data_analysis_subset_grpID_all)
    summary(res_mod_all) 

    ### Fit Jackson's model to test for inconsistency 
    res_mod_all_J <- rma.mv(effect_size, V_list,
                              mods = ~ FF + FF.RS + NL.FF.RS + NL.RS + NL.SE.FF.RS + NL.SE.RS + NL.SE.VF.RS + RS + SE.RS + VF.FF.RS + VF.RS - 1, # The "treatment" left out (BAU) becomes the reference level for the comparisons 
                              random = list(~ 1 | record_id/es_id, ~ domain | record_id, ~ contrast_id | record_id),
                              rho=0.60, phi=1/2,
                              data=NMA_data_analysis_subset_grpID_all)
    summary(res_mod_all_J)

    ### Estimate all pairwise differences between treatments
    contr <- data.frame(t(combn(names(coef(res_mod_all)), 2)))
    contr <- contrmat(contr, "X1", "X2")
    rownames(contr) <- paste(contr$X1, "-", contr$X2)
    contr <- as.matrix(contr[-c(1:2)])
    sav <- predict(res_mod_all, newmods=contr)
    ...
    print(lt_info_df3)
    write_csv(lt_info_df3, 'nma_league_table_all.csv')
    #write_xlsx(lt_info_df3, 'nma_league_table_all.xlsx')
    
    ### Compute p-values
    contr <- data.frame(t(combn(c(names(coef(res_mod_all)),"BAU"), 2))) # add "BAU" to contrast matrix / Likely to remove this from output/forest plot
    contr <- contrmat(contr, "X1", "X2", last="BAU", append=FALSE)
    b <- c(coef(res_mod_all),0) # add 0 for 'BAU' (the "reference treatment" excluded from the mods argument of the rma.mv function executing the NMA above)
    vb <- bldiag(vcov(res_mod_all),0) # add 0 row/column for 'BAU' (the "reference treatment" excluded from the mods argument of the rma.mv function executing the NMA above)
    ...
    res_mod_all_df <- tidy(res_mod_all, conf.int = TRUE)
    pscores_df <- cbind(term = rownames(pscores), as.data.frame(pscores))
    res_mod_all_pscore <- res_mod_all_df %>% left_join(pscores_df, by = c("term"))
    res_mod_all_pscore <- res_mod_all_pscore %>% rename(intervention = term, se = std.error, zval = statistic, pval = p.value, ci.lb = conf.low, ci.ub = conf.high,  Pscore = V1)
    res_mod_all_pscore
    
    ### Create forest plot using metafor's built-in function
    # forest(coef(res_mod_all), diag(vcov(res_mod_all)), slab=sub(".", " ", names(coef(res_mod_all)), fixed=TRUE),
    ...
    ### Create forest plot using ggplot
      ...
      res_mod_all_pscore <- res_mod_all_pscore %>% arrange(desc(Pscore))
      ...
      # res_mod_all_pscore_forest <- ggplot(res_mod_all_pscore, aes(x= estimate, y= intervention, xmin= ci.lb, xmax= ci.ub)) +
      #   geom_hline(aes(yintercept = intervention, colour = colour), size=15) +
      #   geom_pointrange(shape = 22, fill = "black", size = res_mod_all_pscore$num_contrasts/5) +
      #   geom_text(label = res_mod_all_pscore$num_contrasts, hjust = 0.5, vjust = 2.25, colour = "black", size =9, family= "Times New Roman") +
      ...
      #   scale_y_discrete(limits = rev(res_mod_all_pscore$intervention)) +
      ...
      # res_mod_all_pscore_forest
          
      res_mod_all_pscore_forest <- ggplot(res_mod_all_pscore, aes(x= estimate, y= intervention, xmin= ci.lb, xmax= ci.ub)) +
        geom_hline(aes(yintercept = intervention, colour = colour), size=15) +
        geom_pointrange(shape = 22, fill = "black", size = res_mod_all_pscore$num_contrasts/9) +
        geom_text(label = res_mod_all_pscore$num_contrasts, hjust = 0.5, vjust = 2.25, colour = "black", size =5.75, family= "Times New Roman") +
        geom_vline(xintercept = 0, linetype = 3) +
        xlab("Difference in Standardized Mean Change (compared to Control)") +
        labs(caption = "*Values under points indicate number of contrasts                                                        ") +
        ylab("Intervention Bundle") +
        ...
        scale_colour_identity() +
        scale_y_discrete(limits = rev(res_mod_all_pscore$intervention)) +
        theme(axis.title.x = element_text(face = "bold", size=23, family= "Times New Roman"), #x-axis title
              plot.caption = element_text(size = 18, family= "Times New Roman"), #plot caption (x-axis)
              axis.title.y = element_text(face = "bold", size=23, hjust= 0.44, family= "Times New Roman"), #y-axis title
              axis.text.y = element_text(size=25, hjust=0.5), #the intervention bundles
              axis.text.x = element_text(size = 20, family= "Times New Roman")) #x-axis tick values
      res_mod_all_pscore_forest      
      
      #### Next create data table for merging with above plot with estimates and confidence intervals combined in one column
      res_mod_all_pscore2 <- res_mod_all_pscore
      round_digits <- function(x) {
        round(x, digits = 2)
      }
      convert_to_character <- function(x) {
      ...
      res_mod_all_pscore2[c("estimate","Pscore","ci.lb","ci.ub")] <- lapply(res_mod_all_pscore2[c("estimate","Pscore","ci.lb","ci.ub")], round_digits)
      res_mod_all_pscore2[c("estimate","Pscore","ci.lb","ci.ub")] <- lapply(res_mod_all_pscore2[c("estimate","Pscore","ci.lb","ci.ub")], as.character)
      res_mod_all_pscore2$ci.lb <- paste("(", res_mod_all_pscore2$ci.lb, " -", sep= "")
      res_mod_all_pscore2$ci.ub <- paste(res_mod_all_pscore2$ci.ub, ")", sep= "")
      res_mod_all_pscore2 <- res_mod_all_pscore2 %>% unite(estimate_cis, estimate, ci.lb, ci.ub, sep= " ", remove = FALSE )
      print(res_mod_all_pscore2)
          
      LfLabels1<-data.frame(x=c(1), 
                            y=c(11.5),
                            lab=c("Estimate (95% CI)"))
      LfLabels1      
      data_table1 <- ggplot(data = res_mod_all_pscore2, aes(x, y = intervention)) +
        geom_hline(aes(yintercept = intervention, colour = colour), size = 15) + 
        geom_text(aes(x = 1, label = estimate_cis), size= 7.25, family= "Times New Roman") + 
      ...
      LfLabels2<-data.frame(x=c(1), 
                            ...
                            lab=c("P-score"))
      LfLabels2      
      data_table2 <- ggplot(data = res_mod_all_pscore2, aes(x, y = intervention)) +
        geom_hline(aes(yintercept = intervention, colour = colour), size = 15) + 
        geom_text(aes(x = 1, label = Pscore), size= 7.25, family= "Times New Roman") +
      ...
      aligned_plots <- align_plots(res_mod_all_pscore_forest, data_table1, data_table2, align = "h")
      final_fp_nma_all <- grid.arrange(grobs = aligned_plots, nrow= 1, widths= c(2.5,0.75,0.5))
      final_fp_nma_all
    ...
    ###       groups for the creation of the (weighted) edges of the network graph.
      ...
      NMA_data_analysis_subset2 <- NMA_data_analysis_subset_grpID_all %>% dplyr::select(record_id, contrast_id, intervention_prelim, comparison_prelim, domain, measure_name, es_id, effect_size)
      ...
      NMA_data_analysis_subset_grpID_all %>% count(record_id, contrast_id)
      NMA_data_analysis_subset3 <- NMA_data_analysis_subset_grpID_all %>% distinct(record_id, contrast_id, .keep_all = TRUE)
      ...
      num_students_all_long3
      #num_students_all_long4 <- num_students_all_long3 %>% mutate(sum_num_students_bundle2= sum_num_students_bundle)
      num_students_all_long4 <- num_students_all_long3 %>% mutate(sum_num_students_bundle2= if_else((intervention_comparison=="NL+SE+VF+RS" | intervention_comparison=="SE+RS"),sum_num_students_bundle*3,sum_num_students_bundle))
      num_students_all_long4 <- num_students_all_long4 %>% mutate(sum_num_students_bundle2= if_else((intervention_comparison=="FF" | intervention_comparison=="NL+SE+RS" | intervention_comparison=="NL+SE+VF+RS" | intervention_comparison=="VF+RS" | intervention_comparison=="VF+FF+RS"),sum_num_students_bundle*2,sum_num_students_bundle2))
      num_students_all_long4 <- num_students_all_long4 %>% mutate(sum_num_students_bundle2= if_else((intervention_comparison=="BAU"),sum_num_students_bundle*0.75,sum_num_students_bundle2))
      num_students_all_long4 <- num_students_all_long4 %>% mutate(dist=c(0,1.5,0,1.95,2.95,2.5,-2,-2,0,-1.5,-2,-1.5))
      num_students_all_long4 <- num_students_all_long4 %>% mutate(color=c("lightgray","royalblue1","burlywood1","red","green","mediumpurple1","darkorange","maroon1","azure1","lavenderblush2","yellow","pink"))
      num_students_all_long4 <- num_students_all_long4 %>% mutate(intervention_comparison= ifelse(intervention_comparison == "BAU", "Control", intervention_comparison))
      num_students_all_long4
      
      plot(g, edge.curved=FALSE, edge.width=E(g)$weight,
           layout=layout_in_circle(g, order=c("BAU","FF","FF+RS","NL+FF+RS","NL+RS","NL+SE+FF+RS","NL+SE+RS","NL+SE+VF+RS","RS","SE+RS","VF+FF+RS","VF+RS")),
           vertex.size=(num_students_all_long4$sum_num_students_bundle2)/50, vertex.color=num_students_all_long4$color, 
           vertex.label.color="black", vertex.label.font=2, vertex.label=num_students_all_long4$intervention_comparison, vertex.label.dist=num_students_all_long4$dist)  

      # num_students_all_long4
      # num_students_all_long5 <- num_students_all_long4 %>% mutate(sum_num_students_bundle3= if_else(intervention_comparison=="BAU",sum_num_students_bundle*5000,sum_num_students_bundle))
      # num_students_all_long5
      # 
      # plot(g, edge.curved=FALSE, edge.width=E(g)$weight,
      #      layout=layout_in_circle(g, order=c("BAU", "RS","NL+RS","FF","FF+RS")),
      #      vertex.size=log((num_students_all_long5$sum_num_students_bundle3))*2.5, vertex.color=num_students_all_long4$color,
...
# Combine final analysis files by domain
      NMA_data_analysis_subset_grpID_final <- bind_rows(NMA_data_analysis_subset_grpID_all, NMA_data_analysis_subset_grpID_allR)
